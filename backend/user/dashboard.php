...
// Function to fetch loyalty program data
function fetch_loyalty_program_data($conn)
{
    $query = "SELECT image, program_name, loyalty_type, challenge_description FROM loyalty_program";
    $stmt = $conn->prepare($query);
    $stmt->execute();
    $result = $stmt->get_result();
    if (!$result) {
        error_log("Loyalty program query failed: " . $stmt->error);
        return false;
    }
    return $result;
}
...
$loyalty_result = fetch_loyalty_program_data($conn);
