...
if (isset($_POST["challenge_id"]) && !empty($_POST["challenge_id"])) {
    ...
    if (empty($input_name)) {
        $program_name_err = "Please enter a challenge name.";
    } else {
    ...
    if (empty($program_name_err) && empty($challenge_type_err) && empty($company_err) && empty($description_err) && empty($start_err) && empty($end_err) && empty($image_err) && empty($contact_person_err) && empty($contact_email_err) && empty($location_err) && empty($prizes_err) && empty($deadline_err) && empty($sponsors_err) && empty($judges_err) && empty($rules_err) && empty($website_err)) {
        ...
        $sql = "UPDATE challenges SET program_name=?, challenge_type=?, company_name=?, challenge_description=?, start_date=?, end_date=?, image=?, contact_person=?, contact_email=?, location=?, prizes=?, registration_deadline=?, sponsors=?, judges=?, rules_guidelines=?, website=? WHERE challenge_id=?";

        if ($stmt = mysqli_prepare($conn, $sql)) {
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssssssssssssssi", $param_program_name, $param_challenge_type, $param_company, $param_description, $param_start_date, $param_end_date, $param_image, $param_contact_person, $param_contact_email, $param_location, $param_prizes, $param_deadline, $param_sponsors, $param_judges, $param_rules, $param_website, $param_id);
